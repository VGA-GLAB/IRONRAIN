...
namespace Enemy.Control
{
    ...
    public class EnemyController : MonoBehaviour, IDamageable
    {
        ...
        private void Awake()
        {
            ...
            // 装備を自身の状態と連携できるように参照を渡す。
            if (TryGetComponent(out IEquipment equip)) equip.RegisterOwner(_blackBoard);
            else Debug.LogWarning($"武器のコンポーネントが無い:{gameObject.name}");
        ...
        /// 外部から敵の行動を制御する。
        /// /// </summary>
        public void Order(EnemyManager.Order order)
        {
        ...
        public void Attack()
        {
            _overrideOrder.Buffer(EnemyManager.Order.Type.Attack);
        }
        ...
        public void Pause()
        {
            _overrideOrder.Buffer(EnemyManager.Order.Type.Pause);
        }
        ...
        public void Resume()
        {
            _overrideOrder.Buffer(EnemyManager.Order.Type.Resume);
        }
